...
// Handles augmented reality tracking
...
- (void)updateLocations
{
	...
	for (ARGeoCoordinate *item in [self coordinates]) {
		...
		if ([self shouldDisplayCoordinate:item]) {
            ...
            float ychange = loc.y * scaleFactor;    //change y position of marker so farther = taller
            
			//[markerView setFrame:CGRectMake(loc.x - width / 2.0, loc.y, width, height)]; old code
            [markerView setFrame:CGRectMake(loc.x - width / 2.0, ychange, width, height)];
